...
func TestDriverResourcesProjectsKnownCard(t *testing.T) {
	...
	resources := DriverResources("worker-a", device.InventorySnapshot{Devices: []device.InventoryDevice{item}})
	got := resources.Pools["worker-a"].Slices[0].Devices[0]
	assertStringAttribute(t, got, AttributeDeviceID, "pci-0000:01:00.0")
	assertStringAttribute(t, got, AttributeNodeName, "worker-a")
	assertStringAttribute(t, got, AttributeFabricID, "fabric-a")
	assertIntAttribute(t, got, AttributeTensixCoreCount, 70)
	quantity := got.Capacity[resourceapi.QualifiedName(DeviceCapacityMemoryBytes)].Value
...
func TestDriverResourcesPublishesUnknownCardWithoutProfileData(t *testing.T) {
	...
	resources := DriverResources("worker-a", device.InventorySnapshot{Devices: []device.InventoryDevice{item}})
	got := resources.Pools["worker-a"].Slices[0].Devices[0]
	assertStringAttribute(t, got, AttributeChipSeries, "quasar")
	assertStringAttribute(t, got, AttributeCardSeries, "q950x")
	if _, ok := got.Attributes[resourceapi.QualifiedName(AttributeTensixCoreCount)]; ok {
...
func TestDriverResourcesFiltersAndChunksDevices(t *testing.T) {
	...
	resources := DriverResources("worker-a", device.InventorySnapshot{Devices: devices})
	...
	resources = DriverResources("worker-a", device.InventorySnapshot{Devices: devices[:1]})
...
func TestMatchesDeviceClass(t *testing.T) {
	tests := []struct {
		...
		{name: "generic unknown", class: GenericDeviceClassName, chip: "quasar", card: "q950x", want: true},
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			if got := MatchesDeviceClass(testCase.class, testCase.chip, testCase.card); got != testCase.want {
