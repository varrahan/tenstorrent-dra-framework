...
if [[ -r "$pidfile" ]] && kill -0 "$(<"$pidfile")" 2>/dev/null; then
  echo "ttsim VM is already running with PID $(<"$pidfile")" >&2
  exit 1
fi
...
echo "ttsim VM started with PID $(<"$pidfile")"
echo "SSH: ssh -i $HOME/.ssh/ttsim_vm_ed25519 -p $ssh_port ubuntu@127.0.0.1"
